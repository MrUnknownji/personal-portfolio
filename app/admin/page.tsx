...
export default function AdminPage() {
	...
	useEffect(() => {
		...
		if (linkedinAuth === "success") {
			...
			setMessage(
				"LinkedIn authentication successful! Your profile picture and name will now be fetched from LinkedIn."
			);
			setError("");
			...
			switch (errorParam) {
				...
				default:
					setError(
						`LinkedIn authentication failed: ${errorParam || "Unknown error"}`
					);
			}
		}
	}, []);
					...
					{isAuthenticated ? (
						<div className="p-4 bg-green-100 text-green-800 rounded-md">
							...
							<div className="mt-4 p-3 bg-blue-50 text-blue-800 rounded-md text-sm">
								...
								<p className="mt-1">
									Due to LinkedIn API restrictions, only your profile picture
									and name can be fetched automatically. Other metrics like
									connections, endorsements, and posts will use the fallback
									data you&apos;ve provided.
								</p>
								<p className="mt-2">
									LinkedIn has significantly restricted their API access over
									the years and no longer provides these metrics through their
									standard API.
								</p>
							</div>
							<p className="mt-4 text-sm">
								<strong>Note:</strong> The access token is temporarily stored
								and will be lost when the server restarts. In a production
								environment, you would store this token securely.
							</p>
						</div>
					) : (
						<div>
							...
							<p className="mb-4">
								Connect your LinkedIn account to display your real profile
								picture and name on your portfolio.
							</p>
							...
							<div className="mt-4 p-4 bg-blue-50 text-blue-800 rounded-md">
								...
								<p className="mt-1">
									Your LinkedIn application needs to be verified and authorized
									before you can use OAuth. Follow these steps:
								</p>
								...
								<p className="mt-2">
									Until this process is complete, your portfolio will use
									fallback LinkedIn data.
								</p>
							</div>
						</div>
					)}
