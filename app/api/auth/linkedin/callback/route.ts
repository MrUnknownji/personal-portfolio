...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/admin?linkedin_auth=error&error=${error}`);
    }
    
    // Verify authorization code
    if (!code) {
      console.error('Authorization code not provided');
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/admin?linkedin_auth=error&error=no_code`);
    }
    ...
    if (!clientId || !clientSecret) {
      ...
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/admin?linkedin_auth=error&error=no_credentials`);
    }
    ...
    if (!tokenResponse.ok) {
      ...
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/admin?linkedin_auth=error&error=token_exchange`);
    }
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/admin?linkedin_auth=success`);
  } catch (error) {
    console.error('LinkedIn callback error:', error);
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/admin?linkedin_auth=error&error=unknown`);
  }
} 