...
const DialogActions = ({ onClose, isEmailCopied }: DialogActionsProps) => {
	...
	const handleProjectsButtonHover = (isEntering: boolean) => {
		...
		const ctx = gsap.context(() => {
			gsap.to(projectsButtonRef.current, {
				...
				opacity: isEntering
					? ANIMATION_CONFIG.HOVER.OPACITY.HOVER
					: ANIMATION_CONFIG.HOVER.OPACITY.NORMAL,
	...
	return (
		...
		>
          ...
          focus:ring-primary/50 relative overflow-hidden transform-gpu"
				onClick={onClose}
          ...
          focus:ring-2 focus:ring-primary/50 relative overflow-hidden backdrop-blur-sm transform-gpu"
				onClick={onClose}
			...
			>
				{isEmailCopied ? (
					<>
						<FiCheck className="w-5 h-5 mr-2 text-primary transform-gpu" />
				...
				<div
					...
					className="absolute inset-0 bg-gradient-to-r from-primary/0 via-primary/5 to-primary/0 opacity-0 transform-gpu"
				/>
			</button>
		</div>
	);
