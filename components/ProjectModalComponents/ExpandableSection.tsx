...
export const ExpandableSection = ({
	...
	const toggleExpand = useCallback(() => {
		...
		if (!isExpanded) {
			...
			// Collapsing
			if (contentRef.current.children.length > 0) {
				timelineRef.current.to(Array.from(contentRef.current.children), {
					opacity: 0,
					y: 10,
	...
	return (
		...
		>
			...
			>
				<h3 className="text-lg font-semibold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent cursor-pointer">
					{title}
				</h3>
				<div ref={chevronRef} style={{ willChange: "transform" }}>
