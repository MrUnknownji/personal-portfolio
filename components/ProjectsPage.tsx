...
let projectDataPromise: ReturnType<typeof importProjectData> | null = null;

function importProjectData() {
  return import("@/data/data");
}

function loadProjectData() {
  projectDataPromise ??= importProjectData();
  return projectDataPromise;
}

function preloadProjectExperience() {
  void Promise.all([loadProjectData(), loadProjectModal()]);
}
...
export default function ProjectsPage({
  ...
  const openProject = useCallback(async (projectId: number) => {
    const { selectedProjects } = await loadProjectData();
  ...
  useEffect(() => {
    ...
    const nextProjectIds = orderedProjectIds(projects, targetProjectIds);
  ...
  useEffect(() => {
    ...
    const schedulePreload = () => {
      if (typeof window.requestIdleCallback === "function") {
        idleCallbackId = window.requestIdleCallback(
          () => preloadProjectExperience(),
          { timeout: 2000 },
        );
      } else {
        fallbackTimeoutId = setTimeout(preloadProjectExperience, 800);
      }
    };
    ...
    if (document.documentElement.dataset.portfolioReady === "true") {
      ...
      window.addEventListener("portfolio:initial-ready", schedulePreload, {
        once: true,
      });
    }

    return () => {
      window.removeEventListener("portfolio:initial-ready", schedulePreload);
      if (
        idleCallbackId !== null &&
  ...
  useEffect(() => {
    const handleOpenProject = (event: Event) => {
      ...
      if (requestedProject) {
        void openProject(requestedProject.id);
      }
    };
          ...
          {displayedProjects.map((project) => (
            <div key={project.id} className="project-card-container">
              <ProjectCard
                ...
                onIntent={preloadProjectExperience}
              />
            </div>
          ))}
        </div>
